...
/**
 *
 * <h2>${classObject.getName()} business delegate</h2>
 *
 * <p>
...
public class ${classObject.getName()}Service 
#else
public abstract class ${classObject.getName()}Service 
#end##if ( $classObject.isAbstract() == false )
#if ( $classObject.hasParent() == true )
extends ${classObject.getParentName()}Service {
#else
extends BaseService {
...
//************************************************************************
     ...
     * Default Constructor 
     */
    public ${classObject.getName()}Service()  {
	...
	*/
	public static ${className}Service get${className}Instance() {
...
#set( $alias = ${singleAssociation.getAssignToCommandAlias()} )
	...
	public void assign${roleName}( ${alias} command ) throws ProcessingException {
		...
		${childType}Service childDelegate 	= ${childType}Service.get${childType}Instance();
		${className}Service parentDelegate = ${className}Service.get${className}Instance();			
...
#set( $queryHandlerType = $handler.getQueryHandlerType().toString() )
	...
	protected ${className} load( UUID id ) throws ProcessingException {
		${lowercaseClassName} = ${className}Service.get${className}Instance().get${className}( new ${className}FetchOneSummary(id) );	
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(${className}Service.class.getName());
