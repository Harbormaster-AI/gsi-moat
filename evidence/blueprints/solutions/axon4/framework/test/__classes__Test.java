 ...
 * <p>
 * Junit test class for entity ${className}.  Handle the full lifecycle as it relates to persistence. Leverages
 * the ${className}BusinessDelegate and a ${className}Subscriber to test everything under a MVC framework which includes the
 * ${className}Aggregate and related commands, queries, and events.
 * </p>
 #generatedFrom()
 *
 * <h3>Services</h3>
...
#set( $roleName = $multiAssociation.getRoleName() )
				subscriber.${roleName}Subscribe(${lowercaseClassName}Id).upda-tes().subscribe(
						  successValue -> LOGGER.info(successValue.toString()),
						  error -> LOGGER.warning(error.getMessage()),
