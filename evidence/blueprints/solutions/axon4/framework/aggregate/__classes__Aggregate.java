...
/**
 * <h2>${className}Aggregate</h2>
 * <p>
 * The ${className} aggregate is the authoritative consistency boundary for all
 * branch-related operations. It validates incoming commands, enforces
 * business rules, maintains the integrity of the ${className} domain, and emits
 ...
 *       <li>${singleAssociation.getAssignToCommandAlias()}</li>
 *       <li>${singleAssociation.getUnAssignFromCommandAlias()}</li>
#end
 *  </ul>
 *  <ul>
#foreach( $multiAssociation in $classObject.getMultipleAssociations() )
 *       <li>${multiAssociation.getAddToCommandAlias()}</li>
 *       <li>${multiAssociation.getRemoveFromCommandAlias()}</li>
 *   </ul>
 *
 * <p>
 ...
 *      <ul>
 *      <li>${classObject.getCreateEventAlias()}</li>
 *      <li>${classObject.getUpdateEventAlias()}</li>
 *      <li>${classObject.getDeleteEventAlias()}</li>
 #set( $includeComposites = false )
 #foreach( $singleAssociation in $classObject.getSingleAssociations( ${includeComposites} ) )
 *       <li>${singleAssociation.getAssignToEventAlias()}</li>
 *       $<li>{singleAssociation.getUnAssignFromEventAlias()}</li>
 #end
 #foreach( $multiAssociation in $classObject.getMultipleAssociations() )
 *       <li>${multiAssociation.getAddToEventAlias()}</li>
 *       <li>${multiAssociation.getRemoveFromEventAlias()}</li>
 #end
 *        </ul>
 *
